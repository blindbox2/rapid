...
def test_get_data_type_mapping_invalid(session: Session):
    ...
    assert str(
        exception_info.value) == f"404: data_type_mapping with ID: 1 not found."
...
def test_soft_delete_data_type_mapping(session: Session):
    ...
    db_data_type_mapping = data_type_mapping_crud.get_model_on_id(
        session, model_id=1)
...
def test_name_source_id_unique(session):
    created_table = data_type_mapping_crud.create_model(
        session, valid_data_type_mapping)
